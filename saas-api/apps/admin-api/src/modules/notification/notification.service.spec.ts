...
describe('NotificationService', () => {
  ...
  describe('pushToUser', () => {
    it('应推送通知给指定用户', async () => {
      await service.pushToUser(1, { type: 'approval', title: '审批通知', content: '您的进件已通过' })
    ...
    it('网关异常时不应抛出', async () => {
      ...
      await expect(service.pushToUser(1, { type: 'system', title: 'X', content: 'Y' })).resolves.not.toThrow()
    })
  })

  describe('pushToTenant', () => {
    it('应推送通知给租户', async () => {
      await service.pushToTenant(1, { type: 'announcement', title: '公告', content: '内容' })
    ...
    it('网关异常时不应抛出', async () => {
      ...
      await expect(service.pushToTenant(1, { type: 'system', title: 'X', content: 'Y' })).resolves.not.toThrow()
    })
  })

  describe('pushAnnouncement', () => {
    it('应推送公告给租户', async () => {
      await service.pushAnnouncement(1, { id: 1, title: '公告', content: '内容' })
    ...
    it('网关异常时不应抛出', async () => {
      mockGateway.emitToTenant.mockImplementation(() => { throw new Error('连接断开') })
      await expect(service.pushAnnouncement(1, { id: 1, title: 'X', content: 'Y' })).resolves.not.toThrow()
    })
  })

  describe('pushApprovalStatus', () => {
    it('应推送审批状态给用户', async () => {
      await service.pushApprovalStatus(1, { applicationId: 100, status: '审批通过', title: '审批结果' })
      expect(mockGateway.emitToUser).toHaveBeenCalledWith(1, 'notification', expect.objectContaining({
        type: 'approval',
  ...
  describe('pushSupplement', () => {
    it('应推送补件通知', async () => {
      await service.pushSupplement(1, { applicationId: 100, title: '补件通知' })
  ...
  describe('pushSigning', () => {
    it('应推送签约通知', async () => {
      await service.pushSigning(1, { applicationId: 100, title: '签约通知' })
      expect(mockGateway.emitToUser).toHaveBeenCalled()
    })
  })
  ...
  describe('pushLoan', () => {
    it('应推送放款通知', async () => {
      await service.pushLoan(1, { applicationId: 100, amount: 150000 })
      expect(mockGateway.emitToUser).toHaveBeenCalled()
    })
  })
