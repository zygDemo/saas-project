...
export class ReadingController {
  ...
  @Get('categories')
  @ApiOperation({ summary: '获取书籍分类列表' })
  async getCategories(@Request() req) {
    return this.readingService.getCategories(req.user.tenantId);
  }

  @Post('categories')
  @ApiOperation({ summary: '创建书籍分类' })
  async createCategory(@Request() req, @Body() dto: CreateBookCategoryDto) {
    return this.readingService.createCategory(req.user.tenantId, dto);
  }

  @Put('categories/:id')
  @ApiOperation({ summary: '更新书籍分类' })
  async updateCategory(@Param('id') id: number, @Request() req, @Body() dto: UpdateBookCategoryDto) {
  ...
  async removeFromBookshelf(@Param('bookId') bookId: number, @Request() req) {
  ...
  async getReadingProgress(@Param('bookId') bookId: number, @Request() req) {
    return this.readingService.getReadingProgress(req.user.sub, req.user.tenantId, +bookId);
  }

  @Post('progress')
  @ApiOperation({ summary: '保存阅读进度' })
  async saveReadingProgress(@Request() req, @Body() dto: SaveReadingProgressDto) {
  ...
  async getReviews(@Request() req, @Query() query: ReviewQueryDto) {
  ...
  async getHotBooks(@Request() req, @Query('limit') limit?: number) {
    return this.readingService.getHotBooks(req.user.tenantId, limit);
  }

  @Get('recommend')
  @ApiOperation({ summary: '获取推荐书籍' })
  async getRecommendBooks(@Request() req, @Query('limit') limit?: number) {
