/**
 * 农历（中国传统阴阳历）工具模块
 * 支持 1900-2100 年的农历 ↔ 公历转换
 *
 * 数据来源：寿星万年历算法
 */

// ==================== 农历数据表 ====================

/**
 * 1900-2100 年的农历数据
 * 每个元素为一个 16 进制数，含义：
 * - bit 0-3: 闰月月份（0=无闰月）
 * - bit 4-15: 每月大小月（1=30天，0=29天），从正月开始
 * - bit 16: 闰月大小（0=29天，1=30天，仅当有闰月时有效）
 */
const LUNAR_INFO: number[] = [
  0x04bd8, 0x04ae0, 0x0a570, 0x054d5, 0x0d260, 0x0d950, 0x16554, 0x056a0, 0x09ad0, 0x055d2,
  0x04ae0, 0x0a5b6, 0x0a4d0, 0x0d250, 0x1d255, 0x0b540, 0x0d6a0, 0x0ada2, 0x095b0, 0x14977,
  0x04970, 0x0a4b0, 0x0b4b5, 0x06a50, 0x06d40, 0x1ab54, 0x02b60, 0x09570, 0x052f2, 0x04970,
  0x06566, 0x0d4a0, 0x0ea50, 0x06e95, 0x05ad0, 0x02b60, 0x186e3, 0x092e0, 0x1c8d7, 0x0c950,
  0x0d4a0, 0x1d8a6, 0x0b550, 0x056a0, 0x1a5b4, 0x025d0, 0x092d0, 0x0d2b2, 0x0a950, 0x0b557,
  0x06ca0, 0x0b550, 0x15355, 0x04da0, 0x0a5b0, 0x14573, 0x052b0, 0x0a9a8, 0x0e950, 0x06aa0,
  0x0aea6, 0x0ab50, 0x04b60, 0x0aae4, 0x0a570, 0x05260, 0x0f263, 0x0d950, 0x05b57, 0x056a0,
  0x096d0, 0x04dd5, 0x04ad0, 0x0a4d0, 0x0d4d4, 0x0d250, 0x0d558, 0x0b540, 0x0b6a0, 0x195a6,
  0x095b0, 0x049b0, 0x0a974, 0x0a4b0, 0x0b27a, 0x06a50, 0x06d40, 0x0af46, 0x0ab60, 0x09570,
  0x04af5, 0x04970, 0x064b0, 0x074a3, 0x0ea50, 0x06b58, 0x05ac0, 0x0ab60, 0x096d5, 0x092e0,
  0x0c960, 0x0d954, 0x0d4a0, 0x0da50, 0x07552, 0x056a0, 0x0abb7, 0x025d0, 0x092d0, 0x0cab5,
  0x0a950, 0x0b4a0, 0x0baa4, 0x0ad50, 0x055d9, 0x04ba0, 0x0a5b0, 0x15176, 0x052b0, 0x0a930,
  0x07954, 0x06aa0, 0x0ad50, 0x05b52, 0x04b60, 0x0a6e6, 0x0a4e0, 0x0d260, 0x0ea65, 0x0d530,
  0x05aa0, 0x076a3, 0x096d0, 0x04afb, 0x04ad0, 0x0a4d0, 0x1d0b6, 0x0d250, 0x0d520, 0x0dd45,
  0x0b5a0, 0x056d0, 0x055b2, 0x049b0, 0x0a577, 0x0a4b0, 0x0aa50, 0x1b255, 0x06d20, 0x0ada0,
  0x14b63, 0x09370, 0x049f8, 0x04970, 0x064b0, 0x168a6, 0x0ea50, 0x06b20, 0x1a6c4, 0x0aae0,
  0x092e0, 0x0d2e3, 0x0c960, 0x0d557, 0x0d4a0, 0x0da50, 0x05d55, 0x056a0, 0x0a6d0, 0x055d4,
  0x052d0, 0x0a9b8, 0x0a950, 0x0b4a0, 0x0b6a6, 0x0ad50, 0x055a0, 0x0aba4, 0x0a5b0, 0x052b0,
  0x0b273, 0x06930, 0x07337, 0x06aa0, 0x0ad50, 0x14b55, 0x04b60, 0x0a570, 0x054e4, 0x0d160,
  0x0e968, 0x0d520, 0x0daa0, 0x16aa6, 0x056d0, 0x04ae0, 0x0a9d4, 0x0a4d0, 0x0d150, 0x0f252,
  0x0d520,
]

const LUNAR_BASE_YEAR = 1900
const LUNAR_BASE_DATE = new Date(1900, 0, 31) // 农历1900年正月初一

// ==================== 工具函数 ====================

/** 获取农历某年的总天数 */
function lunarYearDays(year: number): number {
  let sum = 348 // 12个月 × 29天
  const info = LUNAR_INFO[year - LUNAR_BASE_YEAR]
  for (let i = 0x8000; i > 0x8; i >>= 1) {
    sum += (info & i) ? 1 : 0
  }
  return sum + leapDays(year)
}

/** 获取农历某年闰月的天数 */
function leapDays(year: number): number {
  const info = LUNAR_INFO[year - LUNAR_BASE_YEAR]
  if (info & 0xf) {
    return (info & 0x10000) ? 30 : 29
  }
  return 0
}

/** 获取农历某年闰月月份（0=无闰月） */
function leapMonth(year: number): number {
  return LUNAR_INFO[year - LUNAR_BASE_YEAR] & 0xf
}

/** 获取农历某年某月的天数 */
function lunarMonthDays(year: number, month: number): number {
  const info = LUNAR_INFO[year - LUNAR_BASE_YEAR]
  return (info & (0x10000 >> month)) ? 30 : 29
}

// ==================== 核心转换 ====================

export interface LunarDate {
  year: number
  month: number
  day: number
  isLeap: boolean
}

/**
 * 农历转公历
 * @param lunarYear 农历年
 * @param lunarMonth 农历月（1-12）
 * @param lunarDay 农历日
 * @param isLeap 是否闰月
 * @returns 公历日期 { year, month, day }
 */
export function lunarToSolar(lunarYear: number, lunarMonth: number, lunarDay: number, isLeap = false): { year: number; month: number; day: number } {
  // 计算距离基准日期的天数
  let offset = 0

  // 年
  for (let y = LUNAR_BASE_YEAR; y < lunarYear; y++) {
    offset += lunarYearDays(y)
  }

  // 月
  const leap = leapMonth(lunarYear)
  let isAfterLeap = false
  for (let m = 1; m < lunarMonth; m++) {
    if (m === leap && !isAfterLeap) {
      offset += leapDays(lunarYear)
      isAfterLeap = true
    }
    offset += lunarMonthDays(lunarYear, m)
  }

  // 如果是闰月
  if (isLeap && lunarMonth === leap) {
    offset += lunarMonthDays(lunarYear, lunarMonth)
  }

  // 日
  offset += lunarDay - 1

  // 加上基准日期
  const result = new Date(LUNAR_BASE_DATE.getTime() + offset * 24 * 60 * 60 * 1000)
  return {
    year: result.getFullYear(),
    month: result.getMonth() + 1,
    day: result.getDate(),
  }
}

/**
 * 公历转农历
 * @param year 公历年
 * @param month 公历月（1-12）
 * @param day 公历日
 * @returns 农历日期
 */
export function solarToLunar(year: number, month: number, day: number): LunarDate {
  const target = new Date(year, month - 1, day)
  let offset = Math.floor((target.getTime() - LUNAR_BASE_DATE.getTime()) / (24 * 60 * 60 * 1000))

  // 计算农历年
  let lunarYear = LUNAR_BASE_YEAR
  let yearDays = lunarYearDays(lunarYear)
  while (offset >= yearDays) {
    offset -= yearDays
    lunarYear++
    yearDays = lunarYearDays(lunarYear)
  }

  // 计算农历月
  const leap = leapMonth(lunarYear)
  let lunarMonth = 1
  let isLeapMonth = false
  let monthDays: number

  if (leap > 0) {
    // 有闰月的年份
    for (let m = 1; m <= 12; m++) {
      monthDays = lunarMonthDays(lunarYear, m)
      if (offset < monthDays) {
        lunarMonth = m
        break
      }
      offset -= monthDays

      // 闰月
      if (m === leap) {
        const leapDaysCount = leapDays(lunarYear)
        if (offset < leapDaysCount) {
          lunarMonth = m
          isLeapMonth = true
          break
        }
        offset -= leapDaysCount
      }
    }
  } else {
    // 无闰月
    for (let m = 1; m <= 12; m++) {
      monthDays = lunarMonthDays(lunarYear, m)
      if (offset < monthDays) {
        lunarMonth = m
        break
      }
      offset -= monthDays
    }
  }

  return {
    year: lunarYear,
    month: lunarMonth,
    day: offset + 1,
    isLeap: isLeapMonth,
  }
}

/**
 * 获取农历月份名称
 */
export function getLunarMonthName(month: number, isLeap = false): string {
  const names = ['正', '二', '三', '四', '五', '六', '七', '八', '九', '十', '冬', '腊']
  return (isLeap ? '闰' : '') + names[month - 1] + '月'
}

/**
 * 获取农历日期名称
 */
export function getLunarDayName(day: number): string {
  const tens = ['初', '十', '廿', '三十']
  const ones = ['', '一', '二', '三', '四', '五', '六', '七', '八', '九', '十']
  if (day === 10) return '初十'
  if (day === 20) return '二十'
  if (day === 30) return '三十'
  return tens[Math.floor(day / 10)] + ones[day % 10]
}

/**
 * 获取完整的农历日期字符串
 */
export function getLunarDateString(year: number, month: number, day: number, isLeap = false): string {
  return `${year}年${getLunarMonthName(month, isLeap)}${getLunarDayName(day)}`
}
