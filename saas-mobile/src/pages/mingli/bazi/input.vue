  ...
  <view class="page">
    <view class="night-head">
      ...
      <view class="head-copy">
        ...
        <view class="time-ripple" aria-hidden="true"><view class="ripple-dot" /></view>
    </view>

    <view class="paper-body">
      <view class="intro-line" aria-hidden="true"><text>✦</text><text>每一刻时空，都有独特的五行印记</text><text>✦</text></view>
      <view class="form-scroll">
        <person-form v-model="form" :show-gender="true" :show-lunar-toggle="true" :date-range="dateRange" />

        <button class="submit-btn" :disabled="!canSubmit" hover-class="tap-active" aria-label="开启八字命盘" @tap="submit">
          <view class="btn-orbit" aria-hidden="true" /><text class="btn-star" aria-hidden="true">✦</text><text>开启命盘</text><text class="btn-arrow" aria-hidden="true">→</text>
        </button>
        <view class="tips" role="note" aria-label="排盘说明">
...
import { paiPan } from '@/common/mingli/bazi'
import { setBaziState } from '@/common/mingli/state'
import MysticSky from '@/components/mystic-sky/mystic-sky.vue'
import type { PersonFormData } from '@/components/mingli/person-form.vue'
import PersonForm from '@/components/mingli/person-form.vue'

interface BaziInputForm extends PersonFormData {
  gender: 'male' | 'female'
  isLunar: boolean
}

const form = ref<BaziInputForm>({ birthDate: '', birthHour: 12, gender: 'male', isLunar: false })
...
function submit() {
  ...
  const result = paiPan(year, month, day, form.value.birthHour, form.value.gender)
  const timeLabel = SHI_CHEN_LIST.find(item => item.value === form.value.birthHour)?.label || '时辰不详'
  ...
  const query = `birthDate=${encodeURIComponent(solarDate)}&birthHour=${form.value.birthHour}&gender=${form.value.gender}`
