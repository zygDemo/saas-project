...
.paper-section { position: relative; z-index: 8; margin-top: -2rpx; padding: 40rpx 24rpx calc(40rpx + env(safe-area-inset-bottom)); background: linear-gradient(135deg, rgba(104, 76, 34, .035) 25%, transparent 25%) 0 0/18rpx 18rpx, #efe4cd; }
...
.card-action { margin-top: 18rpx; display: flex; align-items: center; color: #9a7026; font-size: 24rpx; font-weight: 600; }.liuyao-card .card-action { color: #e0bd64; }.arrow { font-size: 28rpx; margin-left: 10rpx; }
...
.hexagram-art { position: absolute; right: 32rpx; top: 58rpx; width: 210rpx; display: flex; flex-direction: column-reverse; align-items: center; }.mini-yao { width: 150rpx; height: 11rpx; background: #e4c36d; box-shadow: 0 0 12rpx rgba(224, 189, 95, .35); margin-top: 14rpx; }.mini-yao:first-child { margin-top: 0; }.mini-yao.broken { background: linear-gradient(90deg,#e4c36d 0 42%,transparent 42% 58%,#e4c36d 58%); }.moon { position: absolute; right: -18rpx; top: -42rpx; color: rgba(237, 205, 128, .25); font-size: 154rpx; animation: breathe 4s infinite; }
...
@keyframes enterUp { from { opacity:0; transform:translateY(28rpx); } to { opacity:1; transform:translateY(0); } } @keyframes spin { to { transform:rotate(360deg); } } @keyframes spinReverse { to { transform:rotate(-360deg); } } @keyframes breathe { 50% { box-shadow:0 0 22rpx rgba(231,194,105,.38); transform:translateX(-50%) scale(1.025); } } @keyframes floatPlanet { 50% { transform:translateY(-9rpx); } }
</style>
