      ...
      <view class="coin-stage" :class="{ shaking: isShaking }">
        <view v-for="(face, index) in coinFaces" :key="index" class="coin" :class="`coin-${index + 1}`">
          <view class="coin-inner">
            ...
            <text class="coin-face">{{ face.label }}</text>
          </view>
        </view>
...
.coin-stage{position:relative;height:250rpx;margin-top:28rpx}.coin{position:absolute;width:130rpx;height:130rpx;border-radius:50%;padding:7rpx;box-sizing:border-box;background:linear-gradient(145deg,#5c3f15 0%,#8a5e24 18%,#b8832e 38%,#d4a83e 52%,#b8832e 68%,#8a5e24 86%,#5c3f15 100%);box-shadow:0 20rpx 24rpx rgba(72,49,17,.28),inset 0 0 0 3rpx rgba(255,239,183,.45),inset 0 -4rpx 8rpx rgba(90,58,10,.3),inset 0 2rpx 4rpx rgba(255,245,196,.3);transition:.3s}.coin-1{left:74rpx;top:50rpx;transform:rotate(-12deg)}.coin-2{left:50%;top:20rpx;margin-left:-65rpx}.coin-3{right:74rpx;top:54rpx;transform:rotate(14deg)}.coin-inner{width:100%;height:100%;position:relative;display:flex;align-items:center;justify-content:center;border-radius:50%;background:radial-gradient(circle at 35% 30%,#cfa850 0%,#b8832e 45%,#9e6c22 100%);color:#3d2306}.coin-rim{position:absolute;inset:0;border-radius:50%;border:2rpx solid rgba(61,35,6,.5);box-shadow:inset 0 0 0 2rpx rgba(255,245,196,.25),0 0 0 1rpx rgba(61,35,6,.35)}.coin-hole{position:absolute;width:32rpx;height:32rpx;background:#8a5e24;box-shadow:inset 0 0 6rpx rgba(61,35,6,.7),0 0 0 1rpx rgba(61,35,6,.5);transform:rotate(45deg)}.coin-char{position:absolute;color:#2a1503;font:700 20rpx STKaiti,KaiTi,serif;text-shadow:0 1rpx 0 rgba(255,245,196,.35)}.coin-char-top{top:16rpx;left:50%;transform:translateX(-50%)}.coin-char-bottom{bottom:16rpx;left:50%;transform:translateX(-50%)}.coin-char-right{right:18rpx;top:50%;transform:translateY(-50%)}.coin-char-left{left:18rpx;top:50%;transform:translateY(-50%)}.coin-face{position:absolute;bottom:8rpx;font-size:14rpx;letter-spacing:2rpx;color:rgba(42,21,3,.7)}.coin-shadow{position:absolute;left:50%;bottom:20rpx;width:380rpx;height:32rpx;margin-left:-190rpx;border-radius:50%;box-shadow:0 0 30rpx 14rpx rgba(55,38,17,.18)}.coin-stage.shaking .coin-1{animation:coinToss .32s linear infinite}.coin-stage.shaking .coin-2{animation:coinToss .27s .05s linear infinite reverse}.coin-stage.shaking .coin-3{animation:coinToss .35s .1s linear infinite}
.divine-btn{position:relative;display:flex;align-items:center;justify-content:center;height:108rpx;padding:0;border:0;border-radius:54rpx;color:#fff8e7;background:linear-gradient(90deg,#c9a34d 0%,#a67b2d 16%,#1e3456 55%,#0f2240 100%);box-shadow:0 18rpx 34rpx rgba(19,41,70,.3),0 0 0 1rpx rgba(225,190,102,.5),inset 0 0 0 2rpx rgba(255,248,231,.2),inset 0 2rpx 0 rgba(255,255,255,.12);font:700 31rpx STKaiti,KaiTi,serif;letter-spacing:5rpx;overflow:hidden;transition:transform .2s,box-shadow .2s;animation:subtlePulse 4s ease-in-out infinite}.divine-btn::after{border:0}.divine-btn::before{content:'';position:absolute;inset:0;background:linear-gradient(90deg,transparent 0%,rgba(255,255,255,.22) 50%,transparent 100%);transform:translateX(-100%);animation:shine 3.5s ease-in-out infinite}.divine-btn:active:not([disabled]){transform:translateY(3rpx) scale(.985);box-shadow:0 10rpx 22rpx rgba(19,41,70,.24),0 0 0 1rpx rgba(225,190,102,.45),inset 0 0 0 2rpx rgba(255,248,231,.16)}.divine-btn[disabled]{opacity:.5;background:linear-gradient(90deg,#5a5a5a,#3a3a3a 50%,#5a5a5a);box-shadow:none;animation:none}.divine-btn[disabled]::before{display:none}.btn-star{margin:0 18rpx;color:#fff8e7;font-size:28rpx;text-shadow:0 0 10rpx rgba(255,248,231,.55);animation:twinkle 2.4s ease-in-out infinite}.btn-star:last-child{animation-delay:-1.2s}.action-row{height:62rpx;display:flex;align-items:center;justify-content:center;color:#8d7751;font-size:21rpx}.ritual-tip{display:flex;gap:18rpx;padding:24rpx 26rpx;border-top:1rpx solid rgba(157,114,42,.3);border-bottom:1rpx solid rgba(157,114,42,.3)}.ritual-tip>text{width:56rpx;height:56rpx;line-height:56rpx;text-align:center;border:1rpx solid #a67b30;border-radius:50%;color:#9a712b;font:32rpx/56rpx STKaiti,serif}.ritual-tip view{flex:1}.tip-title,.tip-copy{display:block}.tip-title{font:700 26rpx STKaiti,serif;color:#2a3a55}.tip-copy{margin-top:6rpx;color:#6e6353;font-size:21rpx;line-height:1.6}
@keyframes coinToss{0%{transform:translateY(0) rotateY(0) rotate(0)}50%{transform:translateY(-52rpx) rotateY(180deg) rotate(14deg)}100%{transform:translateY(0) rotateY(360deg) rotate(0)}}@keyframes formYao{from{opacity:0;transform:scaleX(.15)}to{opacity:1;transform:scaleX(1)}}@keyframes breathe{50%{box-shadow:0 0 90rpx rgba(218,172,70,.28);transform:scale(1.04)}}@keyframes shine{0%{transform:translateX(-100%)}40%{transform:translateX(100%)}100%{transform:translateX(100%)}}@keyframes subtlePulse{0%,100%{box-shadow:0 18rpx 34rpx rgba(19,41,70,.3),0 0 0 1rpx rgba(225,190,102,.5),inset 0 0 0 2rpx rgba(255,248,231,.2)}50%{box-shadow:0 20rpx 40rpx rgba(19,41,70,.36),0 0 0 1rpx rgba(225,190,102,.6),inset 0 0 0 2rpx rgba(255,248,231,.24)}}@keyframes twinkle{0%,100%{opacity:.6;transform:scale(.9)}50%{opacity:1;transform:scale(1.15)}}
</style>
