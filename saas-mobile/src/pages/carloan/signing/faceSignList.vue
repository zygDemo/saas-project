...
function buildSignCenterUrl(item) {
  const signRouteQuery = buildSignRouteQuery({
    creditOrderId: item.creditOrderId || item.id || "",
    uuid: item.uuid || "",
    customerName: item.customerName || "",
    customerPhone: item.customerPhone || "",
    signStatus: item.signStatus || "CONFIRMING_AMOUNT",
  });
  return buildRoute(APP_ROUTES.carloan.signing.signCenter, signRouteQuery);
}
