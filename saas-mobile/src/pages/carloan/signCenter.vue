...
function goStep(step) {
  ...
  if (customerName.value) {
    query.push(`customerName=${encodeURIComponent(customerName.value)}`);
  }
  if (customerPhone.value) {
    query.push(`customerPhone=${encodeURIComponent(customerPhone.value)}`);
  }
  if (step.key === "SIGN_CONTRACT") {
    query.push("type=contract");
    query.push(`name=${encodeURIComponent(customerName.value)}`);
    query.push(`phone=${encodeURIComponent(customerPhone.value)}`);
  }
  uni.navigateTo({
    url: `${step.path}?${query.join("&")}`,
  });
}
