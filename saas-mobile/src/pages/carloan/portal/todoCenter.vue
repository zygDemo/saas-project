...
const statsItems = computed(() => [
  {
    ...
    icon: "folder-upload",
    color: "amber",
    path: APP_ROUTES.carloan.supplement.supplementList,
  },
  {
    key: "signing",
    label: "待签约",
    ...
    icon: "edit",
    color: "green",
    path: APP_ROUTES.carloan.signing.faceSignList,
  },
  {
    key: "approval",
    label: "待审批",
    ...
    icon: "time",
    color: "blue",
    path: APP_ROUTES.carloan.approval.approvalList,
  },
  {
    key: "orders",
    label: "进行中订单",
    ...
    icon: "order",
    color: "purple",
    path: APP_ROUTES.carloan.precheck.orderList,
  },
]);
...
function goToSupplement(item) {
  uni.navigateTo({
    url: buildRoute(APP_ROUTES.carloan.supplement.supplementDetail, buildSupplementRouteQuery({ id: item.id, creditOrderId: item.creditOrderId || "" })),
  });
}

function goToSigning(item) {
  uni.navigateTo({
    url: buildRoute(APP_ROUTES.carloan.signing.signCenter, buildSignRouteQuery({ creditOrderId: item.creditOrderId || item.orderNo || "", uuid: item.uuid || "", customerName: item.customerName || "" })),
  });
}

function goToApproval(item) {
  uni.navigateTo({
    url: buildRoute(APP_ROUTES.carloan.precheck.applyDetail, buildDetailRouteQuery({ id: item.id || item.applicationId })),
  });
}
