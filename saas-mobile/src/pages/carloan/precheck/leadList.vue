      ...
      >
        ...
        <view class="lead-list">
          <view
            ...
            :key="item.uuid"
            class="lead-card"
            :class="`status-${getTagType(item.status)}`" role="button" tabindex="0" @click="goToDetail(item)"
          >
            ...
            <view class="lead-info">
              <view class="info-row">
                ...
                <text
                  class="value phone-value" role="button" tabindex="0" @click.stop="callPhone(item.phone)"
                >
                  {{ item.phone || "-" }}
                </text>
              </view>
...
const goToDetail = (item: LeadItem) => {
  uni.navigateTo({
    url: buildRoute(APP_ROUTES.carloan.precheck.idInfo, buildEntryRouteQuery({ uuid: item.uuid })),
  });
};
