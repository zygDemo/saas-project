...
function buildOrderQuery(order: OrderListViewItem) {
  ...
  return buildDetailRouteQuery({
    ...
    nodeCode: normalizeNodeCode(order?.nodeCode ?? order?.currentNode ?? order?.businessNode),
...
function handleDetailButton(order: OrderListViewItem) {
  ...
  const route =
    ...
    NODE_DETAIL_ROUTE_MAP[nodeCode] || APP_ROUTES.carloan.precheck.applyDetail;
...
function dotClass(item: any) {
  ...
  if (item.approvalStatus.includes("拒绝"))
