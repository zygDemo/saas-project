...
export function useAuthApi() {
  return {
    // 登录
    login: (data: LoginData) => http.post<LoginResult>(LOGIN_URL, data, { meta: { loadingText: '登录中...' } }),
    getUserInfo: () => http.get<UserInfoResult>("/user/info"),
    logout: () => http.post('/auth/logout', undefined, { meta: { loadingText: '退出中...' } }),
    sendSmsCode: (phone: string, type: 'login' | 'register' = 'login') =>
      http.post('/auth/sms/send', { phone, type }, { meta: { loadingText: '发送中...' } }),
    ...
    sendEmailCode: (email: string, type: 'login' | 'register' = 'login') =>
      http.post('/auth/email/send-code', { email, type }, { meta: { loadingText: '发送中...' } }),
    // 邮箱验证码登录
