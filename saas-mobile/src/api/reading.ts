...
export function useReadingApi() {
  return {
    ...
    addToBookshelf: (bookId: number | string) =>
      http.post<ApiResponse<void>>("/reading/bookshelf", { bookId }, { meta: { loadingText: "保存中..." } }),

    /** 移出书架 */
    removeFromBookshelf: (bookId: number | string) =>
      http.delete<ApiResponse<void>>(`/reading/bookshelf/${bookId}`, undefined, { meta: { loadingText: "删除中..." } }),
    ...
    saveProgress: (data: { bookId: number; chapterId: number; progress: number }) =>
      http.post<ApiResponse<void>>("/reading/progress", data, { meta: { loadingText: "保存中..." } }),
    ...
    createReview: (data: { bookId: number; content: string; rating?: number }) =>
      http.post<ApiResponse<void>>("/reading/reviews", data, { meta: { loadingText: "提交中..." } }),

    /** 删除评论 */
    deleteReview: (id: number | string) =>
      http.delete<ApiResponse<void>>(`/reading/reviews/${id}`, undefined, { meta: { loadingText: "删除中..." } }),
    ...
    purchaseChapter: (chapterId: number | string) =>
      http.post<ApiResponse<{ purchased: boolean }>>(`/reading/chapters/${chapterId}/purchase`, undefined, { meta: { loadingText: "购买中..." } }),
    ...
    likeReview: (reviewId: number | string) =>
      http.post<ApiResponse<{ liked: boolean }>>(`/reading/reviews/${reviewId}/like`, undefined, { meta: { loadingText: "处理中..." } }),
    ...
    createNote: (data: { bookId: number; chapterId: number; content: string; startPos?: number; endPos?: number }) =>
      http.post<ApiResponse<void>>("/reading/notes", data, { meta: { loadingText: "保存中..." } }),
  };
}
