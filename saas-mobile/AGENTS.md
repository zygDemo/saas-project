...
> **AI Agent / Codex 端口规则**：除非用户明确要求“启动预览/打开页面/运行 dev server”，不要执行 `pnpm dev`、`pnpm dev:h5`、`uni --mode development` 等常驻开发服务命令。代码验证优先使用一次性命令 `pnpm build:h5:dev`，执行完成后应自动退出。若确实需要预览，优先复用当前项目已经存在的开发服务端口；不要把 `5173` 当成固定端口，也不要因为端口占用反复新开 `5174/5175/其他随机端口`。启动前后都要检查监听端口，确认本次到底占用了哪个端口。如果由 Agent 启动了 dev server，任务结束前必须说明实际端口并主动停止对应 node 进程。
