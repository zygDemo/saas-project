...
export const nodeFieldDefs: Record<number, { source?: string; label: string; fields: string[] }> = {
  ...
  1140: {
    source: '',
    label: '待预审',
    fields: ['status', 'currentNodeName', 'currentStatusName', 'phaseName', 'remark']
  },
  ...
  1310: {
    source: 'customer',
    label: '客户资料',
    fields: ['name', 'phone', 'idCard', 'gender', 'birthDate', 'address']
  },
  ...
  1500: {
    source: 'funder',
    label: '资方终审',
    fields: ['name', 'funderType', 'contactName', 'contactPhone']
  },
  1600: { source: '', label: '签约办理', fields: ['sign'] },
  1610: {
    source: '',
    label: '额度确认',
    fields: ['approvedAmount', 'approvedTerm', 'approvedRate']
  },
    ...
    actions: [
      {
        ...
        visible: (row: Record<string, unknown>) =>
          ['DRAFT', 'PENDING_SUPPLEMENT'].includes(String(row.status))
      },
      {
        name: 'risk-pre-pass',
        label: '风控预审通过',
        type: 'success',
        visible: (row: Record<string, unknown>) =>
          ['SUBMITTED', 'PENDING_RISK_PRE'].includes(String(row.status))
      },
      {
      ...
      },
      {
        ...
        label: '资方预审通过',
        type: 'success',
        visible: (row: Record<string, unknown>) => String(row.status) === 'PENDING_FUNDER_PRE'
      },
      {
    ...
    actions: [
      {
        ...
        type: 'primary',
        visible: (row: Record<string, unknown>) =>
          ['PENDING_SUPPLEMENT', 'FUNDER_PRE_PASSED'].includes(String(row.status))
      }
    ]
  },
  {
    ...
    actions: [
      ...
      },
      {
        ...
        type: 'danger',
        visible: (row: Record<string, unknown>) =>
          ['PENDING_FIRST_REVIEW', 'PENDING_FINAL_REVIEW', 'PENDING_FUNDER_REVIEW'].includes(
            String(row.status)
          )
      },
      {
      ...
      },
      {
        ...
        label: '提交资方审批',
        type: 'primary',
        visible: (row: Record<string, unknown>) => String(row.status) === 'FINAL_REVIEW_PASSED'
      }
    ]
  },
  {
    ...
    actions: [
      {
        ...
        label: '资方通过',
        type: 'success',
        visible: (row: Record<string, unknown>) => String(row.status) === 'PENDING_FUNDER_REVIEW'
      },
      {
        name: 'funder-reject',
        label: '资方拒绝',
      ...
      },
      {
        ...
        type: 'primary',
        visible: (row: Record<string, unknown>) =>
          ['FINAL_REVIEW_PASSED', 'FUNDER_REVIEW_PASSED'].includes(String(row.status))
      }
    ]
  },
  {
    ...
    actions: [
      {
        ...
        type: 'success',
        visible: (row: Record<string, unknown>) =>
          ['PENDING_SIGN', 'SIGNING_PROGRESS'].includes(String(row.status))
      }
    ]
  },
  {
    ...
    actions: [
      ...
      },
      {
        ...
        label: '请款审核通过',
        type: 'success',
        visible: (row: Record<string, unknown>) => String(row.status) === 'LOAN_REQUEST_REVIEWING'
      },
      {
      ...
      },
      {
        ...
        label: 'GPS安装完成',
        type: 'success',
        visible: (row: Record<string, unknown>) => String(row.status) === 'PENDING_DISBURSEMENT'
      },
      {
        name: 'mortgage-done',
        label: '抵押完成',
    ...
    actions: [
      {
        ...
        label: '结清归档',
        type: 'success',
        visible: (row: Record<string, unknown>) => String(row.status) === 'DISBURSED'
      },
      {
...
const fieldLabelMap: Record<string, string> = {
  ...
  idCardValidFrom: '身份证有效期起',
  idCardValidTo: '身份证有效期止',
  maritalStatus: '婚姻状况',
  education: '学历',
  occupation: '职业',
  companyName: '单位名称',
  ...
  contactName: '联系人',
  contactPhone: '联系电话',
  integrationMode: '对接方式',
  creditLimit: '授信额度',
  priority: '优先级',
  supplementReason: '补件原因',
  supplementDeadline: '补件截止',
  idCardFront: '身份证正面',
  idCardBack: '身份证反面',
  vehicleCode: '车辆编码',
  vehicleImgUrl: '车辆照片',
    ...
    .map((f) => {
      ...
      if (Array.isArray(value)) {
        ...
        const display = value
          .map((item: Record<string, unknown>) => {
            ...
            const status = item.status
              ? repayStatusMap[String(item.status)] || String(item.status)
              : ''
            return status
          })
          .filter(Boolean)
        ...
        return {
          ...
          value: (display || `${value.length}项`) + suffix
        }
      }
      ...
      if (f === 'status') {
        ...
        return {
          prop: f,
          label: fieldLabelMap[f] || f,
          value: statusMap[String(value)] || String(value)
        }
      }
...
export function resolveRowPhaseCode(
  ...
  if (
    rowPhase &&
    phaseConfig.some((p) => p.code === rowPhase || p.nodes.some((n) => n === rowPhase))
  ) {
    return rowPhase
  }
