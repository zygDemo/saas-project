...
<script setup lang="ts">
  ...
  import { fetchArticleDetail, fetchAllArticleTypes, fetchCreateArticle, fetchUpdateArticle } from '@/api/article'
  ...
  const uploadImageUrl = `${API_BASE_URL}/common/upload`
  ...
  const getArticleTypes = async () => {
    try {
      const res = await fetchAllArticleTypes()
      articleTypes.value = (res.data || res) as ArticleType[]
    } catch (error) {
  ...
  const getArticleDetail = async () => {
    try {
      ...
      articleType.value = data.typeId
      editorHtml.value = data.content || ''
      cover.value = data.coverImg || ''
      visible.value = data.status === 'PUBLISHED'
    } catch (error) {
  ...
  const addArticle = async () => {
    ...
    try {
      ...
      await fetchCreateArticle({
        ...
        status: visible.value ? 'PUBLISHED' : 'DRAFT'
      })
  ...
  const editArticle = async () => {
    ...
    try {
      const cleanedContent = cleanCodeContent(editorHtml.value)
      await fetchUpdateArticle(Number(route.query.id), {
        title: articleName.value,
        content: cleanedContent,
        typeId: articleType.value,
        coverImg: cover.value,
